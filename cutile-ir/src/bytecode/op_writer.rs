...
pub(super) fn write_op_body(
    ...
    if matches!(
        find_op_attr(op, "rounding_mode"),
        Some(Attribute::Integer(7, _))
    ) {
        require_version(
    ...
    match op.opcode {
        ...
        Exp => {
            ...
            {
                require_version("exp.rounding_mode", BytecodeVersion::V13_3, ctx.version)?;
            }
        ...
        MmaF => {
            ...
            if flag_if_bool_true(op, "fast_acc", 0) != 0 {
                require_version("mmaf.fast_acc", BytecodeVersion::V13_3, ctx.version)?;
            }
            if ctx.version >= super::enums::BytecodeVersion::V13_3 {
        ...
        FToI => {
            ...
            if flags != 0 {
                require_version("ftoi.saturating", BytecodeVersion::V13_4, ctx.version)?;
            }
        ...
        Module => {
            ...
            if find_op_attr(op, "producer").is_some() {
                require_version("module.producer", BytecodeVersion::V13_3, ctx.version)?;
            }
            if ctx.version >= super::enums::BytecodeVersion::V13_3 {
...
fn write_inbounds(
    ...
    if values.iter().any(|&value| value) {
        require_version("inbounds=true", BytecodeVersion::V13_4, ctx.version)?;
    }
    if ctx.version >= BytecodeVersion::V13_4 {
