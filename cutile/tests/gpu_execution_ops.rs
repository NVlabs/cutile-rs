...
// ===========================================================================
// Non-divisible size variants
//
// The tests above all use n=128 with tile=128 (perfectly divisible). The tests
// below re-run the same kernels at n=100 (100 % 128 != 0) to surface any
// issues with partial-tile handling in reduce, scan, broadcast, and iota.
// ===========================================================================

// ---------------------------------------------------------------------------
// 9. Reduce sum — non-divisible
// ---------------------------------------------------------------------------

#[test]
fn execute_reduce_sum_non_divisible() {
    common::with_test_stack(|| {
        let n = 100usize;
        let input = api::copy_host_vec_to_device(&Arc::new(vec![1.0f32; n]))
            .sync()
        ...
        assert!(
            (host[0] - n as f32).abs() < 1e-3,
            "reduce_sum non-div: expected {}, got {}",
            n as f32,
            host[0]
        );
    });
...
// ---------------------------------------------------------------------------
// 10. Reduce max — non-divisible
// ---------------------------------------------------------------------------

#[test]
fn execute_reduce_max_non_divisible() {
    common::with_test_stack(|| {
        let n = 100usize;
        let input_host = Arc::new((0..n).map(|i| i as f32).collect::<Vec<_>>());
        let input = api::copy_host_vec_to_device(&input_host)
        ...
        assert_eq!(host.len(), 1);
        assert!(
            (host[0] - 99.0).abs() < 1e-3,
            "reduce_max non-div: expected 99.0, got {}",
            host[0]
        );
    });
}
...
// ---------------------------------------------------------------------------
// 11. Scan (prefix sum) — non-divisible
// ---------------------------------------------------------------------------

#[test]
fn execute_scan_prefix_sum_non_divisible() {
    common::with_test_stack(|| {
        let n = 100usize;
        let data = api::copy_host_vec_to_device(&Arc::new(vec![1.0f32; n]))
            .sync()
        ...
        for i in 0..n {
            ...
            assert!(
                (host[i] - expected).abs() < 1e-3,
                "scan prefix sum non-div: element {} expected {}, got {}",
                i,
                expected,
                host[i]
            );
...
// ---------------------------------------------------------------------------
// 12. Broadcast + arithmetic — non-divisible
// ---------------------------------------------------------------------------

#[test]
fn execute_broadcast_add_non_divisible() {
    common::with_test_stack(|| {
        let n = 100usize;
        let input_host = Arc::new((0..n).map(|i| i as f32).collect::<Vec<_>>());
        let input = api::copy_host_vec_to_device(&input_host)
        ...
        for i in 0..n {
            ...
            assert!(
                (host[i] - expected).abs() < 1e-3,
                "broadcast add non-div: element {} expected {}, got {}",
                i,
                expected,
                host[i]
            );
...
// ---------------------------------------------------------------------------
// 13. Iota — non-divisible
// ---------------------------------------------------------------------------

#[test]
fn execute_iota_non_divisible() {
    common::with_test_stack(|| {
        let n = 100usize;
        let output = api::copy_host_vec_to_device(&Arc::new(vec![0i32; n]))
            .sync()
        ...
        for i in 0..n {
            assert_eq!(
                host[i], i as i32,
                "iota non-div: element {} expected {}, got {}",
                i, i, host[i]
            );
        }
    });
