...
fn test_empty_kernel_bytecode_roundtrip() {
    ...
    let bytecode = tile_ir::write_bytecode(&module).expect("bytecode serialization failed");
    ...
    // Decode and check.
    match tile_ir::decode_bytecode(&bytecode) {
        Ok(decoded) => {
            println!("=== decoded ===\n{decoded}");
...
fn test_empty_kernel_tileiras() {
    ...
    let bytecode = tile_ir::write_bytecode(&module).unwrap();
    println!(
        "=== Bytecode decode ===\n{}",
        tile_ir::decode_bytecode(&bytecode).unwrap()
    );
...
fn build_add_kernel() -> Module {
    ...
    let tile_ptr_f32 = Type::Tile(TileType {
        shape: vec![],
        element_type: TileElementType::Pointer(Box::new(tile_ir::ir::PointerType {
...
fn test_add_kernel_bytecode_roundtrip() {
    let module = build_add_kernel();
    let bytecode = tile_ir::write_bytecode(&module).expect("bytecode serialization failed");
    let decoded = tile_ir::decode_bytecode(&bytecode).expect("decode failed");
    println!("{decoded}");
...
fn test_add_kernel_save_bytecode() {
    ...
    tile_ir::write_bytecode_to_file(&module, path.to_str().unwrap())
