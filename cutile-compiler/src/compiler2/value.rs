...
//!
//! Uses `tile_ir::ir::Value` (Copy, index-based, no lifetimes).
...
/// A compiled value: wraps a `tile_ir::ir::Value` together with its Rust type, kind, bounds,
/// and metadata.
///
/// Port of old `TileRustValue<'c, 'a>` — all lifetime parameters are gone because
/// `tile_ir::ir::Value` is `Copy` (a u32 arena index) and `TileRustType` is owned.
#[derive(Debug, Clone)]
