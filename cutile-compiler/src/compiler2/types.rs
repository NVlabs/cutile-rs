...
//! `tile_ir::ir::Type::parse()`. This is a transitional pattern — ideally
//! `TileRustType` would store `tile_ir::ir::Type` directly.

use super::tile_rust_type::TileRustType;
use quote::ToTokens;
use syn::FnArg;
use tile_ir::ir::{PointerType, ScalarType, TileElementType, TileType, Type};

/// Convert a `cuda_tile_name` string to a `tile_ir::ir::ScalarType`.
...
/// Returns the pre-parsed `tile_ir::ir::Type` from a `TileRustType`.
///
...
pub fn make_tensor_view_type(element_name: &str, shape: &[i64], strides: &[i64]) -> Option<Type> {
    let scalar = scalar_from_name(element_name)?;
    Some(Type::TensorView(tile_ir::ir::TensorViewType {
        element_type: scalar,
        shape: shape.to_vec(),
