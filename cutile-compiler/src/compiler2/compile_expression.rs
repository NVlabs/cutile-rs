...
use super::value::{BlockTerminator, CompilerContext, TileRustValue};
...
use tile_ir::ir::{Attribute, BlockId, Location, Module, Region};
        ...
        stacker::maybe_grow(STACK_RED_ZONE, STACK_GROW_SIZE, || {
            ...
            match expr {
                Expr::ForLoop(for_expr) => {
                    ...
                    let block_args: Vec<tile_ir::ir::Value> = loop_block_args[1..].to_vec();
                    ...
                    let (for_op_id, result_values) = OpBuilder::new(Opcode::For, self.ir_location(&for_expr.span()))
                ...
                Expr::While(while_expr) => {
                    ...
                    let block_args: Vec<tile_ir::ir::Value> = loop_block_args.clone();
                    ...
                    let (yield_op_id, _) =
                        OpBuilder::new(Opcode::Yield, self.ir_location(&while_expr.span())).build(module);
                    ...
                    let (break_op_id, _) = OpBuilder::new(Opcode::Break, self.ir_location(&while_expr.span()))
                    ...
                    let (condition_check_id, _) = OpBuilder::new(Opcode::If, self.ir_location(&while_expr.cond.span()))
                ...
                Expr::If(if_expr) => {
                    ...
                    let (else_region_id, _else_return_type) = {
                        if let Some((_Else, else_expr)) = &if_expr.else_branch {
                            ...
                            let (yield_op_id, _) = OpBuilder::new(Opcode::Yield, self.ir_location(&if_expr.span()))
                ...
                Expr::Unary(unary_expr) => {
                    ...
                    match &*unary_expr.expr {
                        Expr::Lit(lit_expr) => {
                            ...
                            let (op_result, _tile_ir_ty) =
                                build_constant_op(module, block_id, &lit_string, &cuda_tile_ty);
...
fn cuda_tile_element_type_to_tile_ir(cuda_tile_ty: &str) -> tile_ir::ir::Type {
    use tile_ir::ir::{ScalarType, TileElementType, TileType, Type};
    let scalar = super::types::scalar_from_name(cuda_tile_ty).unwrap_or(ScalarType::I32);
    Type::Tile(TileType {
        shape: vec![],
...
fn build_constant_op(
    module: &mut tile_ir::ir::Module,
    block_id: tile_ir::ir::BlockId,
    lit_string: &str,
    cuda_tile_ty: &str,
) -> (tile_ir::ir::Value, tile_ir::ir::Type) {
    use tile_ir::ir::DenseElements;

    let result_ty = cuda_tile_element_type_to_tile_ir(cuda_tile_ty);
    let data = encode_literal_bytes(lit_string, cuda_tile_ty);

    let (op_id, results) = OpBuilder::new(Opcode::Constant, Location::Unknown)
        ...
        .build(module);
    tile_ir::builder::append_op(module, block_id, op_id);
    (results[0], result_ty)
}
