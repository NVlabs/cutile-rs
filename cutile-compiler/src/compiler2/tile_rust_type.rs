...
//! `tile_ir::ir::Type` at construction time, stored in `tile_ir_ty`.
...
use syn::ItemImpl;
use tile_ir::ir::Type as TileIrType;
...
impl TileRustType {
    ...
    pub(crate) fn from_tile(element_name: &str, shape: &[i32]) -> Option<TileRustType> {
        ...
        let tile_ir_ty = super::types::make_tile_type(element_name, &ir_shape)?;
        let shape_str = if shape.is_empty() {
            element_name.to_string()
        } else {
            let dims = shape.iter().map(|s| s.to_string()).collect::<Vec<_>>().join("x");
    ...
    pub(crate) fn from_scalar_ptr(element_name: &str) -> Option<TileRustType> {
        let scalar = super::types::scalar_from_name(element_name)?;
        let tile_ir_ty = TileIrType::Tile(tile_ir::ir::TileType {
            shape: vec![],
            element_type: tile_ir::ir::TileElementType::Pointer(Box::new(
                tile_ir::ir::PointerType { pointee: scalar },
            )),
        });
        let type_str = format!("!cuda_tile.tile<!cuda_tile.ptr<{element_name}>>");
        let rust_ty = syn::parse_str::<syn::Type>(
            &format!("PointerTile<* mut {element_name}, {{[]}}>"),
        ).ok()?;
        Some(TileRustType {
            kind: Kind::PrimitiveType,
            cuda_tile_name: Some("!cuda_tile.tile".into()),
