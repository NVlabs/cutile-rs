                ...
                match statement {
                    ...
                    Stmt::Expr(expr, semicolon) => match expr {
                        Expr::Continue(_continue_expr) => {
                            ...
                            let (op_id, _) = OpBuilder::new(Opcode::Continue, self.ir_location(&expr.span()))
                        ...
                        Expr::Break(_break_expr) => {
                            ...
                            let loop_carry_values = ctx.unpack_some_vars(loop_carry_var_names)?;
                            let (op_id, _) = OpBuilder::new(Opcode::Break, self.ir_location(&expr.span()))
                                .operands(loop_carry_values.iter().copied())
                                .build(module);
                            append_op(module, block_id, op_id);
                        }
            ...
            if terminator_encountered.is_none() {
                ...
                match ctx.default_terminator {
                    Some(BlockTerminator::Yield) => {
                        ...
                        let loop_carry_values = ctx.unpack_some_vars(&loop_carry_var_names)?;
                        let (op_id, _) = OpBuilder::new(Opcode::Yield, self.ir_location(&block_expr.span()))
                            .operands(
                                loop_carry_values
                                    .iter()
                    ...
                    Some(BlockTerminator::Continue) => {
                        let loop_carry_values = ctx.unpack_some_vars(&loop_carry_var_names)?;
                        let (op_id, _) = OpBuilder::new(Opcode::Continue, self.ir_location(&block_expr.span()))
                            .operands(loop_carry_values.iter().copied())
                            .build(module);
                        append_op(module, block_id, op_id);
                    }
                    Some(BlockTerminator::Return) => {
                        ...
                        let (op_id, _) =
                            OpBuilder::new(Opcode::Return, self.ir_location(&block_expr.span())).build(module);
                        append_op(module, block_id, op_id);
                    }
