...
use super::value::{CompilerContext, Mutability, TileRustValue};
...
use tile_ir::ir::{Attribute, BlockId, Module, Region, Value};
...
/// `tile_ir::ir::Type`.  Tries `types::convert_type` first (handles
...
impl<'m> Compiler2<'m> {
    ...
    pub fn compile_compiler_op_call(
        ...
        match compiler_op_name.as_str() {
            "mma" => {
                ...
                let out_is_float = super::types::scalar_from_name(&out_cuda_tile_element_type)
            ...
            "reduce" => {
                ...
                let tile_rust_result_type = match TileRustType::from_tile(&element_type, &shape) {
                    Some(t) => t,
                    None => {
                        let ty = syn::parse_str::<syn::Type>(&format!("Tile<{element_type}, {{ {shape:#?} }}>")).unwrap();
                        self.compile_type(&ty, generic_vars, &HashMap::new())?.unwrap()
                    }
                };
                let tile_rust_iter_operand_type = match TileRustType::from_scalar_tile(&element_type) {
                    Some(t) => t,
                    None => {
                        let ty = syn::parse_str::<syn::Type>(&format!("Tile<{element_type}, {{ [] }}>")).unwrap();
                        self.compile_type(&ty, generic_vars, &HashMap::new())?.unwrap()
                    }
                };
                ...
                let identity_attr = {
                    let scalar_ty = super::types::scalar_from_name(&element_type)
                        .unwrap_or(tile_ir::ir::ScalarType::I32);
                ...
                match compiler_op_function.as_str() {
                    "scalar_to_tile" => {
                        ...
                        if let Some(t) = TileRustType::from_scalar_tile(&element_type) {
                            ...
                            new_value.ty.rust_ty = syn::parse_str::<syn::Type>(
                                &format!("Tile<{element_type}, {{[]}}>"),
                            ).unwrap();
                        }
                    }
                    "tile_to_scalar" => {
    ...
    fn compile_check_partition_access(
        ...
        for i in 0..len {
            ...
            let (assert_op_id, _) = OpBuilder::new(Opcode::Assert, self.ir_location(&call_expr.span()))
                .attr("message", tile_ir::ir::Attribute::String(message))
