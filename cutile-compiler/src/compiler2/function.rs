...
use super::modules::CUDATileModules;
...
use super::value::{BlockTerminator, CompilerContext, Mutability, TileRustValue};

use tile_ir::builder::{append_op, build_single_block_region, OpBuilder};
use tile_ir::bytecode::Opcode;
use tile_ir::ir::{
    Attribute, DenseElements, FuncType, Module, ScalarType, TileElementType, TileType,
    Type,
};
...
/// Self-sufficient compiler2 struct that emits tile-ir ops directly.
pub struct Compiler2<'m> {
...
impl<'m> Compiler2<'m> {
    ...
    pub(crate) fn ir_location(&self, span: &proc_macro2::Span) -> tile_ir::ir::Location {
        let loc = self.resolve_span(span);
        if loc.is_known() {
            tile_ir::ir::Location::FileLineCol {
                filename: loc.file,
                line: loc.line as u32,
                column: loc.column as u32,
            }
        } else {
            tile_ir::ir::Location::Unknown
        }
    }
    ...
    /// Compile the kernel function into a `tile_ir::Module`.
    ...
    fn compile_entry_function(&self, module: &mut Module) -> Result<tile_ir::ir::OpId, JITError> {
        ...
        for (i, r_param_type) in r_params.iter().enumerate() {
            ...
            match self.compile_type(&r_param_type, generic_vars, &type_params)? {
                Some(ty) => {
                    ...
                    let tile_ir_ty = super::types::convert_type(&ty).ok_or_else(|| {
    ...
    pub(crate) fn compile_constant_from_exact_bounds(
        ...
        let scalar = super::types::scalar_from_name(&const_ty_str).ok_or_else(|| {
            JITError::Generic(format!(
        ...
        let (op_id, results) = OpBuilder::new(Opcode::Constant, self.ir_location(&tr_ty.rust_ty.span()))
