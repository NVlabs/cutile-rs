...
//!
//! let artifacts = KernelCompiler::new(my_module::_module_asts, "my_module", "add")
...
///
/// ```rust,ignore
/// let artifacts = KernelCompiler::new(my_module::_module_asts, "my_module", "tile_math")
///     .generics(vec!["32".into()])
///     .strides(&[("output", &[1])])
///     .target("sm_80")
///     .compile()?;
/// ```
pub struct KernelCompiler<F: Fn() -> Vec<crate::ast::Module>> {
    module_asts_fn: F,
    module_name: String,
...
impl<F: Fn() -> Vec<crate::ast::Module>> KernelCompiler<F> {
    /// Creates a new compiler for the given kernel.
    ///
    /// - `module_asts_fn`: The `_module_asts` function generated by `#[cutile::module]`.
    ...
    pub fn new(module_asts_fn: F, module_name: &str, function_name: &str) -> Self {
        Self {
            module_asts_fn,
    ...
    pub fn compile(self) -> Result<CompileArtifacts, JITError> {
        let module_asts = (self.module_asts_fn)();
