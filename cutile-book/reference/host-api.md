...
Every producer tile block must signal or finish. In the consumer, call
`gdc_wait_tko(None)` and pass its result token to every predecessor-dependent
memory operation (directly or through a later token). Source order alone
does not establish that dependency. The launch signal permits scheduling;
the wait establishes dependency completion before consuming data.
