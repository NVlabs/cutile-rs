...
// Vendored from cuda-oxide's `oxide-artifacts` crate (commit c58c30f2) for the
// shared host-crate migration: `embedded` needs it, and the crate is not on
// crates.io, so a published cuda-core cannot depend on it directly. The
// interchange with the compiler side is object-file bytes, not shared types,
// so a private copy is sound. Retire this module for a real dependency once
// both projects live in one repository. The object-read/object-write feature
// gates of the original are compiled unconditionally here.
