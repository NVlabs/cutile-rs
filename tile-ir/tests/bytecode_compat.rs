...
use tile_ir::bytecode::{Opcode, MAGIC};
use tile_ir::ir::*;
...
fn assert_roundtrip(module: &Module) {
    let bytecode =
        tile_ir::write_bytecode(module).unwrap_or_else(|e| panic!("bytecode write failed: {e}"));
    let decoded = tile_ir::decode_bytecode(&bytecode)
...
fn test_invalid_magic() {
    ...
    let result = tile_ir::decode_bytecode(&data);
...
#[test]
fn test_empty_bytes() {
    let result = tile_ir::decode_bytecode(&[]);
