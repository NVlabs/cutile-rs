...
use tile_ir::bytecode::{BytecodeVersion, Opcode};
use tile_ir::ir::*;
use tile_ir::{decode_bytecode, write_bytecode};
...
fn write_with_explicit_version() {
    use tile_ir::bytecode::write_bytecode_version;

    let module = build_kernel("ver_explicit");

    // Write with the current version explicitly.
    let bytecode =
        write_bytecode_version(&module, BytecodeVersion::CURRENT).expect("write failed");
...
fn reject_unsupported_version() {
    ...
    assert!(
        result.is_err(),
        "should reject version below MIN_SUPPORTED"
    );
}
